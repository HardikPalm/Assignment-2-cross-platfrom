...
export class UploadService {
  ...
  async addTempUpload(item: any) {
    ...
    return await this.sqlService.run(
      this.uploadQuery.addTempFile(
        TableName.Table_Temp_Upload,
        keysUser.join(','),
        valueUser.join(','),
      ),
    );
  ...
  async getTempUpload(item: any) {
    return await this.sqlService.run(
      this.uploadQuery.getTempFile(item.tokenId),
    );
  }
  ...
  async uploadS3(file: any, aws: IAwsConfig) {
    ...
    switch (file.Type) {
      case 'audio':
        {
          bucketS3 = bucketS3 + '/audio';
          filename = filename;
        }
        break;
      case 'playlistImage':
        {
          bucketS3 = bucketS3 + '/playlistImage';
          filename = filename;
        }
        break;
      case 'artistImage':
        {
          bucketS3 = bucketS3 + '/artistImage';
          filename = filename;
        }
        break;
      default: {
