    ...
    async use(req: Request, res: Response, next: NextFunction,) {
        ...
        if (req?.headers?.authorization?.length > 0) {
            try {
                ...
                let user = await this.sqlservice.run(this.userquery.findUserByIDQuery(Number(decoded.deviceId), Number(decoded.userId)))
                ...
                if (user) {
                    ...
                    if (user?.roleType == 'Admin') {
                        ...
                        throw new HttpException('User must be admin only', 403)
                    }
